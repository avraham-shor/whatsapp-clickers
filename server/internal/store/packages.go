...
// ownership WHERE), surfacing as ErrNotFound to the caller via zero rows —
// but the handler's draft check runs first, so that path is unreachable in
// practice. No explicit transaction: both statements are single atomic
